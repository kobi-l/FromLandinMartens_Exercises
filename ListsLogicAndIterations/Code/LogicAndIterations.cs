...
namespace ListsLogicAndIterations.Code
{
	public static class LogicAndInterations
	{
		public static int For_SumTwoListsAndAllValueThatAreNotFour(int[] numberArray, List<int> numberList)
		{
			...
			return sum + numberList.Where(x => x != 4).Sum(); 
		...
		public static int While_SumValuesWhileCurrentValueIsNot4_ThenStopKobi(List<int> numberList)
		{
			//While
			int counter = 0;
			int sum = 0;

			while ((counter + 1) <= numberList.Count && numberList[counter] != 4)
			{
				sum += numberList[counter];
				counter++;
			}
